 ...
 */
 ...
 /*Input modificado para el modelo de Scherrer*/
...
int input_read_parameters(
  ...
  */
  ...
  if (pba->Omega0_scf != 0.){
    ...
    printf(" aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa %g\n",pba->scf_parameters[pba->scf_tuning_index] ); /*mdfy*/
  ...
  /*Aquí modificamos quintessence para incorporar el modelo de Scherrer*/
    if (strncmp("quintessence", string1, strlen("quintessence")) == 0){
          ...
          pba->is_quintessence_smg = _FALSE_; /*mdfy se cambio TRUE por FALSE*/
	...
	flag2=_TRUE_;
	//mdfy
	class_read_double("x0_schm", pba->x0_schm);
	class_read_double("f0_schm", pba->f0_schm);
	class_read_double("f2_schm", pba->f2_schm);
	class_read_double("theta_schm", pba->theta_schm);
	class_read_double("DM_schm", pba->DM_schm);
	class_read_double("h_schm", pba->h_schm);
	class_read_double("V0_schm", pba->V0_schm);
	class_read_double("lambda_schm", pba->lambda_schm);
	
	
	pba->parameters_size_smg = 8;
	//class_read_list_of_doubles("parameters_smg",pba->parameters_smg,pba->parameters_size_smg); mdfy
	
	/*Definimos los parámetros que aparecen en el modelo de Scherrer tales como F0,F2,theta,h y m_phi*/
	
	
    double x0_sch =  pba->x0_schm; /*mdfy*/
    double f0_sch = pba->f0_schm;
    double f2_sch = pba->f2_schm;
    double eq_sch= pba->theta_schm;
    double DM_sch= pba->DM_schm;
    double DE_sch= pba->Omega0_smg - DM_sch;
    double V0 = pba->V0_schm;
   // double h=pba->h_schm;  //parámetro h que ajusta nuestra condición inicial
    /*Modificaciones en los parámetros para agregar un potencial*/
  //  double V0=pba->V0_schm;//parámetro m_phi para el potencial cuadrártico
    //double lambda=pba->lambda_schm;

    // Ahora se definen los parametros f0 y f2 en terminos de Omega_DM_sch y Omega_DE_sch

    //f0_sch=(-1.)*DE_sch * pow(pba->H0,2) ;
    f2_sch=(pba->Omega0_smg* pow(pba->H0,2))/(4.*pow(x0_sch,2)*eq_sch) ;
    // pba->parameters_smg[1]=f0_sch;
    pba->f0_schm=0;

    pba->f2_schm=f2_sch;
    pba->V0_schm = (pba->Omega0_smg - DM_sch)* pow(pba->H0,2);// add 2025
...
//	}//end of no has_dxdy_guess_smg
      }//end of quintessence_monomial or Scherrer
...
//     }
    ...
    if (pba->Omega_smg_debug == 0)
      ...
      class_read_double("shooting_parameter_smg",pba->h_schm);
      
       fprintf( stderr,"aquí voy y todo bien %g\n", pba->h_schm); /*mdfy*/

    // test that the tuning is correct. Se comentó mdfy
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case Omega_smg:
      ...
      if (input_verbose > 2)
      //mdfy
      	printf(" h_schm = %e, Omega_smg = %.3e, %.3e, target = %.2e \n", ba.h_schm,
...
int input_get_guess(double *xguess,
  ...
  * - Version 3: use attractor solution */
      ...
      break;
    case Omega_smg:
        //xguess[index_guess] = ba.parameters_smg[ba.tuning_index_smg];mdfy
        
        xguess[index_guess] = ba.h_schm;
