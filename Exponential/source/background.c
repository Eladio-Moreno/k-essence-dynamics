...
int background_indices(
  ...
  /* Vamos a definir las clase que vamos a necesitar del modelo de Scherrer, siendo la de index_bg_y_sch_smg con pontecial exponencial y comentamos la clase de phi''*/
  ...
  //class_define_index(pba->index_bg_phi_prime_smg,pba->field_evolution_smg,index_bg,1);
   class_define_index(pba->index_bg_y_sch_smg,pba->field_evolution_smg,index_bg,1); /*mdfy*/
   ...
   */
   /*Agregamos al clase pata bi_y_sch_smg y comentamos la clase de bi_phi_prime_smg */
  if (pba->has_smg == _TRUE_){
    class_define_index(pba->index_bi_phi_smg,
		       pba->field_evolution_smg,index_bi,1);
		       
    class_define_index(pba->index_bi_y_sch_smg,
       		       pba->field_evolution_smg,index_bi,1); /*mdfy*/
  ...
  */
  ...
  if (pba->background_verbose > 2) {
	...
	printf("Minimal stability values: cs2 = %g, ct2 = %g, D = %g, M2 = %g \n",pba->min_cs2_smg,pba->min_ct2_smg,pba->min_D_smg,pba->min_M2_smg);
      ...
      if (pba->field_evolution_smg == _TRUE_){/*mdfy comentamos la siguiente linea*/
...
/** - fix initial value of modified gravity
  ...
  if(pba->has_smg == _TRUE_){
	...
	/*En el modelo quintessence_monomial agregaremos el modelo de Sherrer. En esta sección van las condiciones iniciales para la ecuación de K-G.*/
      case quintessence_monomial: /*CONDICIÓN INICIAL*/
      
      	pvecback_integration[pba->index_bi_phi_smg] = 0; /*mdfy*/
      //	pvecback_integration[pba->index_bi_y_sch_smg] = pba->parameters_smg[5]*(((pow(pba->parameters_smg[3]*pow(pba->a_today,3),2./3.))/pow(a,2))-1.)*pba->parameters_smg[0]; //a inicial muy muy chica
      	
      	//mdfy 
      	pvecback_integration[pba->index_bi_y_sch_smg] = pba->h_schm*(((pow(pba->theta_schm*pow(pba->a_today,3),2./3.))/pow(a,2))-1.)*pba->x0_schm; //a inicial muy muy chica
...
//         printf("V_scale %e, i %i \n",V_scale,i);
        ...
        //pvecback_integration[pba->index_bi_phi_prime_smg] = -a*sqrt(pba->parameters_smg[0]*2*rho_rad); mdfy
    ...
    // Note: we are using as input parameters f = phi/sqrt(alpha)
	  ...
	  //pvecback_integration[pba->index_bi_phi_prime_smg] = pba->parameters_smg[0]*pba->H0; mdfy
	break;
	...
	if(pba->attractor_ic_smg == _TRUE_){
	  ...
	  //pvecback_integration[pba->index_bi_phi_prime_smg] = a*xi_start*pow(pba->H0,2)/sqrt(rho_rad); mdfy
	}
	else
    {/* non attractor ICs. Se comentó lo sigueinte. mdfy */
	 // pvecback_integration[pba->index_bi_phi_prime_smg] = a*pba->parameters_smg[0]*pow(pba->H0,2)/sqrt(rho_rad); mdfy
	}
	//phi is irrelevant
	...
	//pvecback_integration[pba->index_bi_phi_prime_smg] = pba->parameters_smg[3]; mdfy
	break;
    ...
    case nkgb:
      ...
      */
       ...
       // pvecback_integration[pba->index_bi_phi_prime_smg] = a*phidot_init ; mdfy
      }
      break;
    ...
    if (pba->field_evolution_smg == _TRUE_){
      if (pba->background_verbose>3)
	//printf(" -> Initial conditions: phi = %e, phi' = %e \n",pvecback_integration[pba->index_bi_phi_smg],pvecback_integration[pba->index_bi_phi_prime_smg]);mdfy
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    if (pba->output_background_smg >= 2){
      ...
      class_store_double(dataptr,pvecback[pba->index_bg_y_sch_smg],pba->field_evolution_smg,storeidx); /*mdfy*/
...
int background_derivs(
  ...
  if (pba->has_smg == _TRUE_){
    if(pba->field_evolution_smg){ /*ECUACIÓN DE K-G para potencial exponencial*/
    
    	//dy[pba->index_bi_phi_smg]=y[pba->index_bi_a]*sqrt(2.*(y[pba->index_bi_y_sch_smg]+pba->parameters_smg[0]));
    	//dy[pba->index_bi_y_sch_smg]=((-1.)*(dy[pba->index_bi_phi_smg]/pba->parameters_smg[2])* pow(pba->H0/pba->h,2)*pba->parameters_smg[6]*pba->parameters_smg[7]*exp((-1.)*pba->parameters_smg[7]*y[pba->index_bi_phi_smg])-12.*pvecback[pba->index_bg_H]*y[pba->index_bi_a]*(pow(y[pba->index_bi_y_sch_smg],2)+y[pba->index_bi_y_sch_smg]*pba->parameters_smg[0]))/(4.*pba->parameters_smg[0]+6.*y[pba->index_bi_y_sch_smg]);/*mdfy*/
    	
    	
    	dy[pba->index_bi_phi_smg]=y[pba->index_bi_a]*sqrt(2.*(y[pba->index_bi_y_sch_smg]+pba->x0_schm));
    	
    	/**
    	dy[pba->index_bi_y_sch_smg]=((-1.)*(dy[pba->index_bi_phi_smg]/pba->f2_schm)* pow(pba->H0/pba->h,2)*pba->V0_schm*pba->lambda_schm*exp((-1.)*pba->lambda_schm*y[pba->index_bi_phi_smg])-12.*pvecback[pba->index_bg_H]*y[pba->index_bi_a]*(pow(y[pba->index_bi_y_sch_smg],2)+y[pba->index_bi_y_sch_smg]*pba->x0_schm))/(4.*pba->x0_schm+6.*y[pba->index_bi_y_sch_smg]);
    	mdfy 2025*/ 
    	
    
    	
    	dy[pba->index_bi_y_sch_smg]=((-1.)*(dy[pba->index_bi_phi_smg]/pba->f2_schm) * pba->V0_schm*pba->lambda_schm*exp((-1.)*pba->lambda_schm*y[pba->index_bi_phi_smg])-12.*pvecback[pba->index_bg_H]*y[pba->index_bi_a]*(pow(y[pba->index_bi_y_sch_smg],2)+y[pba->index_bi_y_sch_smg]*pba->x0_schm))/(4.*pba->x0_schm+6.*y[pba->index_bi_y_sch_smg]);/*mdfy*/
      ...
      //dy[pba->index_bi_y_sch_smg]=((dy[pba->index_bi_phi_smg]/pba->parameters_smg[2])*pba->parameters_smg[6]*y[pba->index_bi_phi_smg]-12.*pvecback[pba->index_bg_H]*y[pba->index_bi_a]*(pow(y[pba->index_bi_y_sch_smg],2)+y[pba->index_bi_y_sch_smg]*pba->parameters_smg[0]))/(4.*pba->parameters_smg[0]+6.*y[pba->index_bi_y_sch_smg]); 
...
int background_gravity_functions(
  ...
  if (pba->field_evolution_smg == _TRUE_) {
    ...
    double a, phi, phi_prime, H, y_sch; /*mdfy*/
    double x0 = pba->x0_schm; // Aqui agregamos el parametro x0 que se da en hi_class.ini
    double x,f,df;
    int n, n_max=100;
    ...
    y_sch=pvecback_B[pba->index_bi_y_sch_smg]; /*mdfy*/
    phi = pvecback_B[pba->index_bi_phi_smg]; /*mdfy*/
    phi_prime = a*sqrt(2.*( y_sch+x0)); /*mdfy*/
    ...
    /*Aquí va la densidad lagrangiana y sus derivadas para el modelo de Scherrer*/
    if (pba->gravity_model_smg == quintessence_monomial) {

      //double N = pba->parameters_smg[0];
      //double V0 = pba->parameters_smg[1];
      double f0_sch = pba->f0_schm; /*mdfy*/
      double f2_sch = pba->f2_schm;
      double theta= pba->theta_schm;
      double V0 = pba->V0_schm * pow(pba->H0,2);  // parámetro V0 del potencial exponencial
      double lambda = pba->lambda_schm; 
      double V_phi; 
      double DM_sch= pba->DM_schm;
      
      f0_sch=0.;
      f2_sch=(pba->Omega0_smg* pow(pba->H0,2))/(4.*pow(x0,2)*theta) ;
      pba->f2_schm=f2_sch;																																																		
      																																																																																														pba->V0_schm = V0;
      //V0 = (pba->Omega0_smg - DM_sch) * pow(pba->H0,2);
      
      //V_phi=V0*pow(pba->H0/pba->h,2.)*exp(((-1.)*lambda*phi));//potencial en un																																										idades de CLASS
      V_phi=V0*exp(((-1.)*lambda*phi));//potencial en unidades de CLASS																																	
      //V_phi=0.5*pow(m_phi,2)*pow(phi,2);
      G2 =(-1.)*f0_sch + f2_sch *pow(y_sch,2)-V_phi;; // G2 para Scherrer
      G2_X = 2.*f2_sch*(y_sch); //G2_X para Scherrer
      G2_XX=2.*f2_sch;
      G2_phi= lambda*V_phi;                  //G2_phi para Scherrer
      //G2_phi=(-1.)*m_phi*phi;
    ...
    pvecback[pba->index_bg_y_sch_smg] = y_sch; /*mdfy*/
...
// 	       pvecback[pba->index_bg_rho_smg]/(pvecback[pba->index_bg_rho_smg]+rho_tot));
  ...
  if ( (pba->skip_stability_tests_smg == _FALSE_) && (pba->parameters_tuned_smg == _TRUE_) && (pba->Omega_smg_debug == 0) ){
      ...
      if ((pba->is_quintessence_smg == _TRUE_)&& (1==2)){ //se modifico para que no entre en este if. mdfy
...
int background_gravity_parameters(
  ...
  // case quintessence_monomial:
	...
	//    pba->parameters_smg[0], pba->parameters_smg[1], pba->parameters_smg[1]*pow(pba->H0/pba->h,2),
     ...
     /*En esta sección se leen los parámetros del modelo de Scherrer los cuales son X0, F0, F2, THETA, Omega_DM_sch, Omega_DE_sch, h del shooting*/
        case quintessence_monomial: /*mdfy*/
     printf("Modified gravity: Scherrer exponential with  parameters: \n");
     printf("-> X0 = %g, F0 = %g, F2 = %g, theta = %g, Omega_DM_sch = %g, Omega_DE_sch = %g,  h = %g, V_0 = %g, , lambda = %g \n",
	    pba->x0_schm, pba->f0_schm, pba->f2_schm,
	    pba->theta_schm, pba->DM_schm,  pba->Omega0_smg- pba->DM_schm, pba->h_schm,pba->V0_schm,pba->lambda_schm);
     break;
