...
int input_read_parameters(
  ...
  */
  ...
  /*Aquí modificamos quintessence para incorporar el modelo de Scherrer*/
	...
	class_read_double("m_phi_schm", pba->m_phi_schm);
    ...
    double m_phi=pba->m_phi_schm;//parámetro m_phi para el potencial cuadrártico
...
//     }
    ...
    if (pba->Omega_smg_debug == 0) //mdfy
      //class_read_double("shooting_parameter_smg",pba->parameters_smg[pba->tuning_index_smg]); comentamos esta clase
      class_read_double("shooting_parameter_smg",pba->h_schm);
      
       fprintf( stderr,"aquí voy y todo bien %g\n", pba->h_schm ); /*mdfy*/

    // test that the tuning is correct. mdfy se comentó esto
	...
	//       errmsg,
	  //     "Tuning index tuning_index_smg = %d is larger than the number of entries %d in parameters_smg. Check your .ini file.",
	    //   pba->tuning_index_smg,pba->parameters_size_smg); mdfy hasta aquí
