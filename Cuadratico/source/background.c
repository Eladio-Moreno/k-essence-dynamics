...
int background_derivs(
  ...
  if (pba->has_smg == _TRUE_){
    if(pba->field_evolution_smg){ /*ECUACIÓN DE K-G*/
    
    	dy[pba->index_bi_phi_smg]=y[pba->index_bi_a]*sqrt(2.*(y[pba->index_bi_y_sch_smg]+pba->x0_schm));
    	
      dy[pba->index_bi_y_sch_smg]=((dy[pba->index_bi_phi_smg]/pba->f2_schm)*pow(pba->m_phi_schm,2)*y[pba->index_bi_phi_smg]-12.*pvecback[pba->index_bg_H]*y[pba->index_bi_a]*(pow(y[pba->index_bi_y_sch_smg],2)+y[pba->index_bi_y_sch_smg]*pba->x0_schm))/(4.*pba->x0_schm+6.*y[pba->index_bi_y_sch_smg]) ; /*mdfy*/
...
int background_gravity_functions(
  ...
  if (pba->field_evolution_smg == _TRUE_) {
    ...
    if (pba->gravity_model_smg == quintessence_monomial) {
      ...
      double m_phi = pba->m_phi_schm;  // parámetro m_phi^2 del potencial cuadrático
      double V_phi; 
...
int background_gravity_parameters(
  ...
  // case quintessence_monomial:
	...
	//    pba->parameters_smg[0], pba->parameters_smg[1], pba->parameters_smg[1]*pow(pba->H0/pba->h,2),
     ...
     printf("Modified gravity: Scherrer with parameters: \n");
     printf("-> X0 = %g, F0 = %g, F2 = %g, theta = %g, Omega_DM_sch = %g, Omega_DE_sch = %g,  h = %g, m_phi = %g \n",
	    pba->x0_schm, pba->f0_schm, pba->f2_schm,
	    pba->theta_schm, pba->DM_schm,  pba->Omega0_smg- pba->DM_schm, pba->h_schm,pba->m_phi_schm);
     break;
